...
class WhatsappHelper(RestChannelHandler):
    @classmethod
    ...
    async def to_bot_input(cls, channel: JBChannel, msg_id: str, msg_obj: ChannelData):
        ...
        if message_type == MessageType.TEXT:
            recieved_message = WhatsappHelper.wa_get_user_text(msg_obj)
        ...
        if message_type == MessageType.INTERACTIVE:
            recieved_message = WhatsappHelper.wa_get_interactive_reply(msg_obj)
        if message_type == MessageType.FORM:
            recieved_message = WhatsappHelper.wa_get_form_reply(msg_obj)
    ...
    @classmethod
    def wa_get_user_text(cls, msg_obj: ChannelData):
        if msg_obj.type == MessageType.TEXT:
            time_stp = msg_obj.timestamp
            data = msg_obj.text["body"]
            return WAMsgInfo(MessageType.TEXT, time_stp, data)
        return {}

    # get audio message from user
    @classmethod
    def wa_get_user_audio(cls, channel: JBChannel, msg_obj: ChannelData):
        if msg_obj.type == MessageType.AUDIO:
            time_stp = msg_obj.timestamp
            data = msg_obj.audio["id"]
            b64audio = WhatsappHelper.wa_download_audio(channel=channel, fileid=data)
            return WAMsgInfo(MessageType.AUDIO, time_stp, b64audio)
        return {}

    @classmethod
    def wa_get_interactive_reply(cls, msg_obj: ChannelData):
        if msg_obj.type == MessageType.INTERACTIVE:
            ...
            return WAMsgInfo(MessageType.INTERACTIVE, time_stp, data)
        return {}

    @classmethod
    def wa_get_form_reply(cls, msg_obj: ChannelData):
    ...
    @classmethod
    def wa_download_audio(cls, channel: JBChannel, fileid: dict):
