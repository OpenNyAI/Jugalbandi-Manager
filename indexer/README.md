...
The indexer service is used to index the given textual data which gets stored as embedded vectors in postgres.

## How to use the Indexer?

1. Start the api and indexer services using the below command:

   ```bash
   ./scripts/run.sh api indexer
   ```

2. Once the services are running, open your browser and navigate to http://0.0.0.0:8000/docs. This will load the Swagger UI, which provides an interface to interact with the available endpoints.

3. In the swagger UI, locate the endpoint `/v1/index-data` under v1 section to send data to indexer. Click on `Try it out` to enable input fields. 

4. Provide the required parameters:
    - indexer_type: Choose between default or r2r.
    - collection_name: Name of the collection to index.
    - chunk_size: Size of each chunk for indexing. (default is 4000)
    - chunk_overlap_size: Overlap size between chunks. (default is 200)
    - Upload the files to be indexed.

5. Once all inputs are provided, click `Execute` button to send the data to the Indexer service.

6. After the indexing is done, the indexer service will respond with **Embeddings have been created for the collection**. You can also confirm the success of the indexing operation by verifying the chunks in the  PostgreSQL table.


## What happens in the Indexer?

The indexing process involves two major steps - preprocessing and chunking. The preprocessed and chunked data are then indexed. Both the steps are explained in detail below.

###  Preprocessing:

The preprocessing step in indexing involves parsing the file before chunking it. The function parse_file() accepts the entire pathname of the file as a parameter and uses it to extract the extension of the file. Depending on what the file extension is, the corresponding functions for parsing the file are called. Currently, the implementation for file parsing has been provided for files of the format **pdf**, **docs**, **xslx** and **json** only.
...
## Some commonly used preprocessing and chunking resources:

### Resource links related to pre-processing and parsing of documents:
...
### Resource links related to chunking of data:

1. [SpaCy: Text Chunking](https://spacy.io/usage/linguistic-features#chunking)
...
2. [NLTK: Chunking in Python (Natural Language Toolkit)](https://www.nltk.org/book/ch07.html)
The NLTK library offers robust support for chunking, including techniques like regular expression-based chunking.

3. [Chunking Text into Sentences or Paragraphs](https://realpython.com/natural-language-processing-spacy-python/#chunking-text)
This tutorial explains how to chunk text into sentences or paragraphs using the SpaCy library.

4. [Latent Dirichlet Allocation (LDA) for Topic Modeling](https://www.machinelearningplus.com/nlp/topic-modeling-gensim-python/)
...
5. [Topic Modeling and Chunking with Gensim](https://radimrehurek.com/gensim/auto_examples/tutorials/run_lda.html)
...
6. [BERT for Sentence Embeddings and Chunking](https://huggingface.co/transformers/model_doc/bert.html)
...
7. [Text Chunking in PDFs with PyPDF2](https://pypdf2.readthedocs.io/en/3.x/)
...
8. [PDFMiner](https://pdfminersix.readthedocs.io/en/latest/)
