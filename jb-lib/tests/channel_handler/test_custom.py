...
with patch("lib.encryption_handler.EncryptionHandler", mock_encryption_handler):
    from lib.channel_handler.custom_channel_handler import CustomChannelHandler
...
test_output_messages = {
    "text_message": (
        ...
        json.dumps(
            {
                "messaging_product": "custom",
                "preview_url": False,
                "recipient_type": "individual",
                "to": "test_custom_identifier",
                "type": "text",
                "text": {"body": "Hello"},
            }
        ),
...
@pytest.fixture
def sample_user():
    return JBUser(identifier="test_custom_identifier", id="sample_user_id")
...
def test_send_message(message, sample_channel, sample_user):
    mock_post = MagicMock()
    with patch("lib.channel_handler.custom_channel_handler.requests.post") as mock_post:
        CustomChannelHandler.send_message(sample_channel, sample_user, message[0])
        mock_post.assert_called_once_with(
            url=f"{sample_channel.url}/v1/messages",
            data=message[2],
            headers={
                "Content-type": "application/json",
                "Authentication": "Bearer decrypted_test_channel_key",
            },
        )
                    ...
                    "changes": [
                        {
                            "value": {
                                "messaging_product": "custom",
                                "metadata": {
                                    "display_bot_name": "custom_bot_1",
                                    "bot_id": "custom_bot_id_1",
                                },
                                "messages": [
                                    {
                                        "from": "test_id_1",
                    ...
                    "changes": [
                        {
                            "value": {
                                "messaging_product": "custom",
                                "metadata": {
                                    "display_bot_name": "custom_bot_2",
                                    "bot_id": "custom_bot_id_2",
                                },
                                "messages": [
                                    {
                                        "from": "test_id_2",
                    ...
                    "changes": [
                        {
                            "value": {
                                "messaging_product": "custom",
                                "metadata": {
                                    "display_bot_name": "custom_bot_3",
                                    "bot_id": "custom_bot_id_3",
                                },
                                "messages": [
                                    {
                                        "from": "test_id_3",
                    ...
                    "changes": [
                        {
                            "value": {
                                "messaging_product": "custom",
                                "metadata": {
                                    "display_bot_name": "custom_bot_4",
                                    "bot_id": "custom_bot_id_4",
                                },
                                "messages": [
                                    {
                                        "from": "test_id_4",
...
def test_get_channel_name():
    assert CustomChannelHandler.get_channel_name() == "custom"
...
def test_is_valid_data(message):
    assert CustomChannelHandler.is_valid_data(message[0]) is True
...
def test_process_message(message):
    channel_data = [msg for msg in CustomChannelHandler.process_message(message[0])]
...
@patch("requests.get")
def test_custom_download_audio(mock_get, sample_channel):
    mock_response = MagicMock()
    mock_response.content = b"audio content"
    mock_get.return_value = mock_response

    with patch("lib.channel_handler.custom_channel_handler.requests.get", mock_get):
        file_content = CustomChannelHandler.download_audio(sample_channel, "file_id")
...
def test_parse_text_message(sample_channel, sample_user, sample_message_text):
    data = CustomChannelHandler.parse_bot_output(
        sample_message_text, sample_user, sample_channel
    )
    assert data["to"] == "test_custom_identifier"
