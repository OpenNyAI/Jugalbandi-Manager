...
from lib.channel_handler.pinnacle_whatsapp_handler import PinnacleWhatsappHandler, ChannelData, User, Message, MessageType, TextMessage, AudioMessage, InteractiveReplyMessage, DialogOption, DialogMessage, Option, JBChannel, JBUser, RestBotInput
        ...
        "entry": [{
            "changes": [{
                "value": {
                    ...
                    "messages": [{"from": "user1", "type": "text", "text": {"body": "hello"}}]
                }
            }]
        }]
    }
...
@pytest.fixture
def jb_channel():
    return JBChannel(app_id="app_id", key="encrypted_key", url="https://api.whatsapp.com")
...
def test_to_message_audio(mock_write_file, mock_download_audio, jb_channel):
    mock_download_audio.return_value = base64.b64encode(b"audio_content").decode("utf-8")
...
def test_parse_text_message(jb_channel, jb_user):
    ...
    result = PinnacleWhatsappHandler.parse_text_message(jb_channel, jb_user, text_message)
    assert result["type"] == "text"
    assert result["text"]["body"] == "hello"

@patch("requests.post")
def test_send_message(mock_post, jb_channel, jb_user, text_message):
    mock_post.return_value = MagicMock(status_code=200, json=lambda: {"messages": [{"id": "message_id"}]})
